...
<<<<<<< HEAD
    ...
    exec('sudo /sbin/wpa_cli -i ' . RASPI_WIFI_CLIENT_INTERFACE . ' list_networks ', $list_networks);
    array_shift($list_networks);
    $configured_networks = [];
    foreach ($list_networks as $nw) {
        $data = explode("\t", $nw);
        $configured_networks[$data[1]] = ['index' => intval($data[0]), 'connected' => isset($data[3])];
    }
...
>>>>>>> 2eb5409c27047c497ff4a1c9482f48b6c52de4ca
    ...
    if (isset($_POST['connect'])) {
        // echo 'sudo /sbin/wpa_cli -i ' . RASPI_WIFI_CLIENT_INTERFACE . ' select_network '.strval($_POST['connect']);
        // exec('sudo wpa_cli -i ' . RASPI_WPA_CTRL_INTERFACE . ' select_network '.strval($_POST['connect']), $data);
        exec('sudo /sbin/wpa_cli -i ' . RASPI_WIFI_CLIENT_INTERFACE . ' select_network '.strval($_POST['connect']), $data);
        // var_dump($data);
        // exit(1);
...
<<<<<<< HEAD
    ...
    foreach ($networks as $ssid => $network) {
        if ($configured_networks[$ssid]) {
            $networks[$ssid]['index'] = $configured_networks[$ssid]['index'];
        } else {
            $networks[$ssid]['index'] = $index;
        }
        $index++;
    } 
=======
